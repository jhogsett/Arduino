...
void move_hand(int new_position){
  int safe_hand_position = hand_position + stepsPerRevolution;
  int safe_new_position = new_position + stepsPerRevolution;

  int diff = 0;
  if(safe_new_position > safe_hand_position){
    // move forward
    diff = safe_new_position - safe_hand_position;
  } else if(safe_new_position < safe_hand_position){
    // move backward
    diff = safe_hand_position - safe_new_position;
  }
  myStepper.step(diff);
}
...
void loop() {
  ...
  second = ++second % 60;
  int hand_position = int((stepsPerRevolution / 60.0) + 0.5);
