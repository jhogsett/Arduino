...
rgb_color palette[NPALETTE] = { pink, ltblue, ltgreen, blue, green, yellow }; //, cyan, magenta, grue, eurg,  egnaro, rurple, white, gray, black }; 
...
//#define RADIUS 7.77861761
//#define RADIUS 7.72055554 - 3
...
void plot(float radius, float angle, rgb_color color, int id = 0){
  ...
  float xpos = int(cos(r) * radius + OFFSET);
  float ypos = int(sin(r) * radius + OFFSET);
...
*/
  ...
  while(true){
    for(int i = 0; i < 360; i += STEP){
      ...
      bool did_draw = plot2(&xpos, &ypos, &lxpos, &lypos, RADIUS, i, random_colors[0]);

      if(did_draw){
        plot(RADIUS, 360.0 - i, white);
        plot(RADIUS, i + 120, random_colors[1]);
        plot(RADIUS, i + 240, random_colors[2]);
        //plot(RADIUS, i + 270, c);
      }

//      for(int j = 1; i < 5; i++){
//        plot(RADIUS, i + 72 * j, c);
//      }

/*      if(lsinx != int(sinx) || lcosy != int(cosy)){
        draw(random_colors[ic++ % NPRETTY_COLORS], sinx, cosy, 0);
        lsinx = int(sinx);
        lcosy = int(cosy);
      }

    plot(RADIUS, i, random_colors[ic++ % NPRETTY_COLORS], 0);
*/
