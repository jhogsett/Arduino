...
{
public:
  NonBlockingSequence(NonBlockingAction* actions, int* times, int num_actions, bool skip_last_action = false, NonBlockingAbort abort_action = NULL){
    _actions = actions;
    _times = times;
    _num_actions = num_actions;
    _abort_action = abort_action;
    _skip_last_action = skip_last_action;
  }
  ...
  bool last_action(){
    return _loop.last_cycle() && _action_iter == _num_actions - 1;
  }
  ...
  bool step(){
    ...
    switch(_state){
      ...
      case STATE_START_ACTION:
        ...
        // if this is the last cycle, last action, and only a delay, skip it 
        if(last_action()){
