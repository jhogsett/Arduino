...
void loop()
{
  ...
  if (keyPad.isPressed())
  {
    ...
    // in repeated calls this will comes be I2C_KEYPAD_NOKEY when isPressed() had just returned true 
    if(key == I2C_KEYPAD_NOKEY){
      // Serial.println("no key");
      return;
    }

    // because it's a non-blocking loop it will come here repeatedly 
    if(key != last_key){
      last_key = key;
