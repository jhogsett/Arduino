...
void loop(void)
{
  int key1 = random(0, 10);
  int key2 = random(0, 10);
  while(key2 == key1)
    key2 = random(0, 10);
  AD1.setFrequency(0, rows[mapr[key1]]);
  AD2.setFrequency(0, cols[mapc[key1]]);
  AD3.setFrequency(0, rows[mapr[key2]]);
  AD4.setFrequency(0, cols[mapc[key2]]);
  delay(DELAY);
