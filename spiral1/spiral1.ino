...
#define MIN_RADIUS_CHANGE 0.01
#define MAX_RADIUS_CHANGE 0.05
...
*/
  ...
  for(int i = 0; i < NUM_OBJECTS; i++){
    a[i] = random_angle(false);
    r[i] = random_radius();
    da[i] = random_dir(MAX_SPEED, MIN_SPEED, false);
    dr[i] = random_dir(MAX_RADIUS_CHANGE, MIN_RADIUS_CHANGE, false);
 }
  ...
  while(true){
    ...
    for(int i = 0; i < NUM_OBJECTS; i++){
      ...
      plot2(&xpos, &ypos, &lxpos, &lypos, r[i], a[i], random_colors[i]);
