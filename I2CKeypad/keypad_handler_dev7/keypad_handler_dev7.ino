...
void loop()
{
  ...
  switch(mode){
    case MODE_WAITING:
      if(NULL != (ch = wait_for_char("ABCD", 1000, STATE_CONTINUED_KEY_PRESS, action_dial, action_undial))){
        switch(ch){
          case 'A':
    ...
    case MODE_CALL_START:
      ...
      if(ch != NULL){
        ...
        if(char_in_chars(ch, "A")){
          ...
          // figure out here the number of digits to look for
    ...
    case MODE_CALL_IN_PROGRESS:
      ...
      if(ch != NULL){
        if(char_in_chars(ch, "A")){
          ...
          add_digit(ch);
          Serial.println(digits);
          // figure out here the number of digits has been reached
        }
      }
      break;
