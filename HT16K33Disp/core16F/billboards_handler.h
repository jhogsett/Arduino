...
{
public:  
  BillboardsHandler(char * buffer, byte num_billboards, Billboard *billboards, char ** templates, int blanking_time, byte home_times);
...
BillboardsHandler::BillboardsHandler(char * buffer, byte num_billboards, Billboard *billboards, char ** templates, int blanking_time, byte home_times){
  _buffer = buffer;
...
void BillboardsHandler::run(unsigned long time, HT16K33Disp * display, char *string){
  ...
  if (!_running) {
    ...
    _current_billboard->begin(times);
  }
