...
long time_to_seconds(long second, long minute, long hour){
  long result = second + (60L * minute) + (3600L * hour); 
  return result; 
}
...
void increment_time(byte &second, byte &minute, byte &hour, bool limit=false, byte seconds=1, byte minutes=0, byte hours=0){
...
bool decrement_time(byte &second, byte &minute, byte &hour, int seconds=1, int minutes=0, int hours=0){
  long total_seconds = time_to_seconds(second, minute, hour);
  total_seconds -= time_to_seconds(seconds, minutes, hours);
  if(total_seconds < 0)
    total_seconds = 0;
...
void render_clock_string(byte seconds, byte minutes, byte hours){
  if(hours < 10)
    sprintf(display_buffer, load_f_string(F("   %1d %02d %02d  ")), hours, minutes, seconds);
  else
    sprintf(display_buffer, load_f_string(F("  %2d %02d %02d  ")), hours, minutes, seconds);
}

int clock_prompt(byte seconds, byte minutes, byte hours){
  ...
  while(true){
    ...
    if(time >= next_second){
      increment_time(clock_second, clock_minute, clock_hour);
      render_clock_string(clock_second, clock_minute, clock_hour);
      display.show_string(display_buffer);
    ...
    if(button_pressed()){
      ...
      if(long_press_state == 1){
        ...
        if(button_states[GREEN_ID])
          increment_time(clock_second, clock_minute, clock_hour, true, 0, 0, 1);
        else if(button_states[AMBER_ID])
          increment_time(clock_second, clock_minute, clock_hour, true, 0, 1, 0);
        else if(button_states[RED_ID]){
          clock_second = 0;
...
int timer_prompt(byte seconds=0, byte minutes=0, byte hours=0){
  ...
  while(true){
    ...
    if(running && time >= next_second){
      if(going_up)
        increment_time(timer_second, timer_minute, timer_hour);
      else
        if(!decrement_time(timer_second, timer_minute, timer_hour))
          running = false;
    ...
    if(button_pressed())
    {
      ...
      if(long_press_state == 1)
      {
        ...
        if(button_states[GREEN_ID]){
          ...
          increment_time(timer_second, timer_minute, timer_hour, false, 0, 1, 0);
        } else if(button_states[RED_ID]){
          increment_time(timer_second, timer_minute, timer_hour, false, 1, 0, 0);
        }
        render_timer_string(timer_second, timer_minute, timer_hour, running);
        display.show_string(display_buffer);
      }
