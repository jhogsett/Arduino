...
//
//  while(true){
//    int num_rects = random(MAX_RECTS - MIN_RECTS) + MIN_RECTS; 
//    
...
//        int ay2 = (y[l] + y[m]) / 2;
//  
//        //line(random_colors[i], ax1, ay1, ax2, ay2);
...
unsigned univ[NUM_ROWS][NUM_COLS];
unsigned _new[NUM_ROWS][NUM_COLS];

void show(rgb_color color, int w, int h)
{
  fade_fast();
  //int (*univ)[w] = u;
...
void evolve(int w, int h)
{
  for_y for_x {
    ...
    if (univ[y][x]) n--;
    _new[y][x] = (n == 3 || (n == 2 && univ[y][x]));
  }
  for_y for_x univ[y][x] = _new[y][x];
}
