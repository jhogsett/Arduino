...
void ValueController::control_up(float sample){
  ...
  if(_trend_detector->fast_mean() < _control_start){
    ...
    if(++_expedite_boost_counter > _expedite_boost){
      _expedite_boost_counter = 0;
      _expedite_boost_value++;  
    }
...
void ValueController::control_down(float sample){
  ...
  set_controller_speed( int( over_boost + ( trend_boost ? _trend_boost : 0 ) + _expedite_boost_value ) );
}
