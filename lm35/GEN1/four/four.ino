...
float mean_sample_temp(){
  ...
  mean_trend_accum += sample;
  last_mean_trend = mean_trend_accum / LONG_WINDOW;

  return last_mean_temp;
}

void prime_temp_sampler(){
  mean_temp_accum = sample_temp() * SHORT_WINDOW;

  for(int i = 0; i <= SHORT_WINDOW; i++){
    mean_sample_temp();
  }

  mean_trend_accum = mean_temp_accum * LONG_WINDOW / SHORT_WINDOW;
}
...
void setup() {
  ...
  while(true){
    ...
    for(int i = 0; i < 2000; i++){
      ...
      Serial.print("Trend = ");
      Serial.println(last_mean_trend > last_mean_temp ? "DOWN" : "UP");
    }
