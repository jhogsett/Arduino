...
float mean_sample_temp(){
  ...
  if(last_long_mean_temp > wmean->mean())
    trend = -0.25 * (last_long_mean_temp - wmean->mean());
    // trend = -1.0;
  else if(last_long_mean_temp < wmean->mean())
    trend = 0.25 * (wmean->mean() - last_long_mean_temp);
    // trend = 1.0;
...
void setup() {
  ...
  wmean = new WindowedMean(SHORT_WINDOW, sample_temp());
