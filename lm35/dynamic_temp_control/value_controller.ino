...
public:
  ...
  const bool UP_CONTROLLER=true;
  const bool DOWN_CONTROLLER=false;
...
int ValueController::surging(void){
  if(!trend_detector->settled())
   if(trend_detector->mean() > 0.0)
    return 1;
  else if(trend_detector->mean() < 0.0)
    return -1;
  return 0;
}

void ValueController::control_up(float sample){
  trend_boost = surging() == -1;
  int under_boost = 0;

  if(trend_detector->fast_mean() < _control_start){
