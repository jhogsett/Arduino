...
void slots_game(){
  ...
  char * text = load_f_string(F("    SHIT   FUCK   PISS   DAMN   FART   POOP    "));

  bool running1, running2, running3 = true;
  while(running1 || running2 || running3){
    ...
    if(running3){
      ...
      if(!running3){
        disp3.show_string("PISS");
      }
    }
  }
  while(!button_pressed());
  while(prompt(load_f_string(F("only a test"))) == -1);
}
