...
// int prompt(char *prompt/*, bool *button_states=NULL*/){
//   unsigned long time;
...
void options_menu(){
  ...
  // sprintf(buffer, load_f_string(F("SOUND   %s")), bet_amounts[current_bet]);
  ...
  //   switch(button_led_prompt(buffer, states)){
  //     case -1:
  //       return;
  //     case 0:
  //       return;
  ...
  //         current_bet = 0;
  //       sprintf(buffer, load_f_string(F("    $%d")), bet_amounts[current_bet]);
  //       disp2.scroll_string(buffer, 1, OPTION_FLIP_SCROLL_TIME);
  //       continue;
  //     case 3:
  //       return;
  //   }
...
void options_mode(){
  ...
  switch(button_led_prompt(load_f_string(F("Sleep   Menu")))){
    case -1:
      return;
    case 0:
      return;
...
#ifdef USE_PROGMEM
    sprintf_P(buffer, "%s%s%s", (char *)pgm_read_ptr(&(words[choice1])), (char *)pgm_read_ptr(&(words[choice2])), (char *)pgm_read_ptr(&(words[choice3])));
#else
    sprintf(buffer, "%s%s%s", words[choice1], words[choice2], words[choice3]);
#endif
    title_prompt(buffer);
...
void time_game(){
  ...
  while(button_led_prompt(load_f_string(F("Press to Start"))) == -1);
  ...
  while(button_led_prompt(buffer) == -1);
}
...
// add to purse on new best reaction time
